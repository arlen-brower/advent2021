...
"""
  ...
  def play_game(_, drawn, [], last, last_board) do
    BingoBoard.calc_win(last_board, drawn, last)
  end

  def play_game([draw | pool], drawn, boards, _, _) do
    drawn = MapSet.put(drawn, draw)
    last_board = hd(boards)
    boards = Enum.reject(
      boards,
      &BingoBoard.check_win( &1, drawn)
    )
    play_game(pool, drawn, boards, draw, last_board)
  end
